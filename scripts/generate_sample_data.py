...
def generate_stock_data(path: Path, rows: int = 2000):
    ...
    with open(path, "w", newline="") as f:
        ...
        base_time = datetime.now(UTC) - timedelta(days=rows/1440) # 1 row per minute
        for i in range(rows):
...
def generate_syslog_data(path: Path, rows: int = 500):
...
def generate_zones_config(path: Path):
