...
@router.get("/alerts")
def export_alerts(format: str = "csv", limit: int = 1000, db: DBSession = Depends(get_db_session)):
    ...
    if format == "json":
        ...
        return PlainTextResponse(
            ...
            headers={
                "Content-Disposition": f"attachment; filename=irmds_alerts_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            },
        )
        ...
        return StreamingResponse(
            ...
            media_type="text/csv",
            headers={
                "Content-Disposition": f"attachment; filename=irmds_alerts_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
            },
        )
