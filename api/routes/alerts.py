...
def list_alerts(
    ...
    if until:
        query = query.filter(AlertRecord.timestamp <= until)
    if session_id:
...
def get_latest_alerts(
    limit: int = Query(10, ge=1, le=100), db: DBSession = Depends(get_db_session)
):
...
def get_alert_stats(db: DBSession = Depends(get_db_session)):
    ...
    # Group by Module
    module_results = (
        db.query(AlertRecord.module, func.count(AlertRecord.id)).group_by(AlertRecord.module).all()
    )
    module_counts = {mod: count for mod, count in module_results}

    # Group by Type
    type_results = (
        db.query(AlertRecord.type, func.count(AlertRecord.id)).group_by(AlertRecord.type).all()
    )
    type_counts = {t: count for t, count in type_results}

    return AlertStatsResponse(
        severity_counts=severity_counts, module_counts=module_counts, type_counts=type_counts
    )


@router.get("/{alert_id}", response_model=AlertSchema)
def get_alert(alert_id: str, db: DBSession = Depends(get_db_session)):
