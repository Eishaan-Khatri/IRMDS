...
def get_all_metrics(
    ...
    for mod_info in registry.list_modules():
        ...
        if mod and mod.status.value == "running":
            ...
            if stats:
                module_metrics.append(ModuleMetricsSchema(module_id=mod_id, metrics=stats))

    return SystemMetricsSchema(system_uptime_seconds=uptime, modules=module_metrics)
...
def get_module_metrics(
    ...
    return ModuleMetricsSchema(module_id=module_id, metrics=stats)
