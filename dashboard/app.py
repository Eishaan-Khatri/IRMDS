...
# ─────────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown("<h2 style='text-align: center; color: white; tracking: 2px;'>🛡️ IRMDS</h2>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: center; color: #8b949e; font-size: 0.8em; margin-bottom: 2rem;'>Intelligent Real-Time Monitoring</p>", unsafe_allow_html=True)
...
try:
    health_res = httpx.get("http://127.0.0.1:8000/health", timeout=2)
...
if not api_status:
    st.error("FastAPI Backend is unreachable. Please start the backend (`irmds start`) to view the dashboard.")
    st.stop()

# Actually, the user should be automatically routed to 01_overview.py or we can render overview here
# Since Streamlit native pages uses the filename `app.py` as Home:
import os
import sys
...
try:
    metrics_res = httpx.get("http://127.0.0.1:8000/metrics").json()
...
if not st.session_state.event_queue:
    st.markdown("<p style='color: #8b949e; text-align: center; padding: 20px;'>Waiting for events from EventBus...</p>", unsafe_allow_html=True)
else:
