...
from dashboard.components.chart_builders import (
    build_gauge_chart,
    build_occupancy_bar,
    build_sparkline,
)
...
try:
    metrics_res = httpx.get(f"{API_URL}/metrics/visual", timeout=2)
    api_online = metrics_res.status_code == 200
    v_metrics = metrics_res.json().get("metrics", {}) if api_online else {}
except Exception:
    api_online = False
    v_metrics = {}
...
zone_counts = v_metrics.get(
    "zone_occupancy", {"Entry_Gate": 2, "Lobby": 5}
...
with col3:
    st.metric(
        "Active Tracks",
        value=active_tracks,
        delta=f"{loiter_alerts} Loitering",
        delta_color="inverse",
    )
    st.write("")
...
with c1:
    ...
    st.plotly_chart(
        build_sparkline(
            list(range(30)), st.session_state.speed_history, "Population Velocity (m/s)"
        ),
        use_container_width=True,
        config={"displayModeBar": False},
    )
...
with c2:
    ...
    st.plotly_chart(
        build_occupancy_bar(zone_counts), use_container_width=True, config={"displayModeBar": False}
    )
